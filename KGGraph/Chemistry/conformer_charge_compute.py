...
class ConformerGeneration:
    ...
    def geometry_optimize(
        ...
        """
        command = f"xtb {input_file} --opt {level} --chrg 0 --parallel {num_cpus}"
        process = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE)
        output, err = process.communicate()

        if process.returncode != 0:
            print(f"Error in xtb: {err}")
            return None
        ...
        temp_files = ["wbo", "xtbrestart", "hessian", "xtbtopo.mol", "xtbopt.log", ".xtboptok"]
        for temp_file in temp_files:
            if os.path.exists(temp_file):
                os.remove(temp_file)

    def run_geometry_optimization_df(
            self, data: DataFrame, 
            smiles_col: str, 
            ID_col: str, 
            xtb: bool = True, 
        ...
        """
        self.smiles2file_df(data, smiles_col, ID_col, self.input_dir, output_format=format)
        if xtb:
            # Iterate over all files in the specified directory
            for file_name in os.listdir(self.input_dir):
                if file_name.endswith("." + format):  # Ensures we are only processing specified format files
                    input_file = os.path.join(self.input_dir, file_name)
                    self.geometry_optimize(input_file, file_name, level, self.output_dir, num_cpus=num_cpus)
                else:
                    print(f"Skipping non-{format} file: {file_name}")
    
    def run_geometry_optimization(
            self, smiles: list or str,
            ID: list = None,
            xtb: bool = True, 
            level: str = 'normal', 
            num_cpus: int = 1, 
        ...
        Returns:
            None
        """
        self.smiles2file(smiles=smiles, ID=ID, lig_dir=self.input_dir, output_format=format)
        if xtb:
            # Iterate over all files in the specified directory
            for file_name in os.listdir(self.input_dir):
                if file_name.endswith("." + format):  # Ensures we are only processing specified format files
                    input_file = os.path.join(self.input_dir, file_name)
                    self.geometry_optimize(input_file, file_name, level, self.output_dir, self.charge_dir, num_cpus=num_cpus)
                else:
                    print(f"Skipping non-{format} file: {file_name}")

if __name__ == "__main__":
    import time
    from conformer_generation import ConformerGeneration
    test_data =    {'check': 'C=CC[O-]',
    ...
    input_dir = "./random_conform"
    output_dir = "./optim_conform"
    charge_dir = "./partial_charge"
    start_time = time.time()
    geo_optim = ConformerGeneration(input_dir, output_dir, charge_dir)
    geo_optim.run_geometry_optimization(smiles = test_data.values(), 
                                        ID = test_data.keys(), 
                                        level='normal', 
                                        num_cpus=-1, 
                                        format="pdb")
    ...
    print(f"Elapsed time for run_geometry_optimization: {elapsed_time} seconds")