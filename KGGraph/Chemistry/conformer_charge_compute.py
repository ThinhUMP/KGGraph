...
class ConformerChargeGeneration:
    ...
    def geometry_charge_compute(
        ...
        geometric_command = f"xtb {input_file} --opt {level} --chrg 0 --parallel {num_cpus}"
        geometric_process = subprocess.Popen(geometric_command, shell=True, stdout=subprocess.PIPE)
        ...
        temp_files = ["wbo", "xtbrestart", "hessian", "xtbtopo.mol", "xtbopt.log", ".xtboptok", "g98.out", "vibspectrum", "xtbopt.log"]
        ...
        if xtb:
            ...
            for file_name in os.listdir(self.input_dir):
                if file_name.endswith("." + format):  # Ensures we are only processing specified format files
                    ...
                    self.geometry_charge_compute(input_file, file_name, level, self.output_dir, self.charge_dir, num_cpus=num_cpus)
                else:
...
if __name__ == "__main__":
    ...
    test_data =    {'Sorafenib': 'CNC(=O)c1cc(Oc2ccc(NC(=O)Nc3ccc(Cl)c(C(F)(F)F)c3)cc2)ccn1',
    ...
    geo_optim.run_geometry_charge(smiles = test_data.values(), 
                                        ID = test_data.keys(), 
                                        level='normal', 
                                        num_cpus=-1, 
                                        format="pdb")
