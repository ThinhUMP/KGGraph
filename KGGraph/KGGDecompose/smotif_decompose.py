...
class SMotifDecomposition:
    ...
    def generate_mark_pattern(mol: Chem.Mol) -> Set[int]:
        ...
        PATTERNS = {
            ...
            "PROPYL": "[C;X4]([#6,#9,#17,#35,#53])([#6,#9,#17,#35,#53])\
                ([#6,#7,#8,#16,#9,#17,#35,#53])",
        }
    ...
    def _is_in_ring_but_not_other(
        ...
        """
        if (
            mol.GetAtomWithIdx(value[0]).IsInRing()
            and not mol.GetAtomWithIdx(value[1]).IsInRing()
        ):
            for ring in rings:
                if value[0] in ring:
                    ring.add(value[1])
    ...
    def _process_carbonyl(
        mol: Chem.Mol, value: List[int], marks: Set[int], pre_cluster: List[List[int]]
    ) -> None:
        ...
        for k in check:
            ...
            if atom.GetSymbol() == "C" and k in marks:
                SMotifDecomposition._handle_carbon_atom(
                    mol, k, value, marks, pre_cluster
                )
            elif k in marks:
