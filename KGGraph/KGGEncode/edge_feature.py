                ...
                for h in range(k+1, len(cliques)):
                    for bond in clique_edges:
                        if (bond[0] == k and bond[1] == h) or (bond[0] == h and bond[1] == k):
                            motif_edge_index.extend([[num_atoms + k, num_atoms + h], [num_atoms + h, num_atoms + k]])
...
def edge_feature(mol, decompose_type, mask_edge, fix_ratio):
    ...
    if not mask_edge:
        ...
        motif_edge_index, edge_index = obj.get_edge_index(
            edge_index_node, obj.num_motif, obj.cliques, 
            obj.num_atoms, obj.clique_edges
        )
    ...
    # If masking is enabled
        ...
        # Get the edge index tensor for the graph and the edge attribute tensor for the graph
        motif_edge_index, edge_index = obj.get_edge_index(
            edge_index_masked, obj.num_motif, obj.cliques, 
            obj.num_atoms, obj.clique_edges
        )
