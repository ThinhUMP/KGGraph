...
def create_test_round_df(args, roc_list, ap_list, f1_list, task_type, training_round):
    ...
    if args.dataset == "tox21":
        ...
        test_metrics_tox21.to_csv(
            f"{args.save_path+task_type}/{args.dataset}/test_metrics_round_{training_round}.csv",
            index=False,
        )
...
def create_train_round_df(
    ...
    train_df.to_csv(
        f"{args.save_path+task_type}/{args.dataset}/train_metrics_round_{training_round}.csv",
        index=False,
    )
...
def average_train_metrics(args, task_type, remove=False):
    ...
    for i in range(1, args.training_rounds + 1):
        file_path = (
            f"{args.save_path+task_type}/{args.dataset}/train_metrics_round_{i}.csv"
        )
...
def average_test_metrics(args, task_type, remove=True):
    ...
    for i in range(1, args.training_rounds + 1):
        file_path = (
            f"{args.save_path+task_type}/{args.dataset}/test_metrics_round_{i}.csv"
        )
        round_metrics = pd.read_csv(file_path)
        dfs = pd.concat([dfs, round_metrics], axis=0)
        if remove:
    ...
    print(
        f"AUC test for {args.dataset} dataset over {args.training_rounds} training rounds: {df_avg.AUC.mean()*100:.2f}±{df_std.AUC.mean()*100:.2f}"
    )
