...
from KGGraph.KGGChem.bond_features import (
    get_bond_type, is_conjugated, is_rotatable, get_stereo, 
    get_bond_polarity, is_bond_in_ring, bond_type_feature
)

class TestBondProperties(unittest.TestCase):
    def setUp(self):
        # Create a molecule that includes various types of bonds
        self.mol = Chem.MolFromSmiles('O=C(C)C=C')  # Contains single, double, and rotatable bonds
    ...
    def test_is_bond_in_ring(self):
        self.assertFalse(is_bond_in_ring(self.single_bond))  # No rings in the test molecule

    def test_bond_type_feature(self):
        bonds = Chem.MolFromSmiles('c1ccccc1').GetBonds()
        self.assertEqual(bond_type_feature(bonds[0]), [1, 0.5, 1, 1])  # Example vector for 'AROMATIC'
        # Test if the function returns correct feature vector
        self.assertEqual(bond_type_feature(self.double_bond), [1, 1, 1, 0])  # Example vector for 'DOUBLE' conjugated
        self.assertEqual(bond_type_feature(self.single_bond), [1, 0, 0, 0])  # Example vector for 'SINGLE' NOT conjugated
        self.assertEqual(bond_type_feature(self.bonds[2]), [1, 0, 1, 0])  # Example vector for 'SINGLE' conjugated
        self.assertEqual(bond_type_feature(self.bonds[3]), [1, 1, 1, 0])  # Example vector for 'DOUBLE' conjugated
